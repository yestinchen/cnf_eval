 ...
 */
public class SimpleFixedOcclusionObjectSequenceGenerator {
    ...
    public SimpleFixedOcclusionObjectSequenceGenerator(int numClass, int averageObjects, int averageDuration,
                                                       int averageDurationSTD) {
        ...
        for (int i =0; i < frameNum; i++) {
            ...
            for (int j = 0; j < currentClassNum; j++) {
                ...
                if (j < currentClasses.size()) {
                    ...
                    for (int k = generatedIds.size() -1 ; k >=0; k--) {
                        GeneratedId id = generatedIds.get(k);
                        // randomly.
                        if(newIds.size() < eachClass) {
                            if (i % 100 == 0){
                    ...
                    if (newIds.size() < eachClass) {
                        int maxK = eachClass - newIds.size();
                        // add the rest.
    ...
    public static void generateFile(int numClass, int averageObjects,
        ...
        String fileName = "./datagen/occfixed-"+numClass+"-"+averageObjects+"-"+averageDuration+"-"+averageDurationSTD+"-"+recordNum+".sequence";
        if (!override) {
        ...
        SimpleFixedOcclusionObjectSequenceGenerator generator = new SimpleFixedOcclusionObjectSequenceGenerator(numClass,
